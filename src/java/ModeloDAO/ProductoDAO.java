...
public class ProductoDAO implements CRUDProducto {
    ...
    @Override
    public Producto list(int id) { //28- Cambiamos la clase
        String sql="select * from producto where IdPro="+id;//29- Cambiamos la tabla
        try {
            con=cn.getConnection();
            ps=con.prepareStatement(sql);
            rs=ps.executeQuery();
    ...
    public boolean eliminar(int id) {
        String sql="delete from producto where IdPro="+id;//36- Cambiar nombre tabla
        try {
            con=cn.getConnection();
            ps=con.prepareStatement(sql);
            ps.executeUpdate();
